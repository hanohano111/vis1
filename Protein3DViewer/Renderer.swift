...
// 临时导入BufferIndex和VertexAttribute，直到全部使用ShaderTypes命名空间
...
let alignedUniformsSize = (MemoryLayout<ShaderTypes.UniformsArray>.size + 0xFF) & -0x100
...
actor Renderer {
    ...
    var uniforms: UnsafeMutablePointer<ShaderTypes.UniformsArray>
    ...
    init(_ layerRenderer: LayerRenderer, appModel: AppModel) {
        ...
        uniforms = UnsafeMutableRawPointer(dynamicUniformBuffer.contents()).bindMemory(to:ShaderTypes.UniformsArray.self, capacity:1)
    ...
    private func updateDynamicBufferState() {
        ...
        uniforms = UnsafeMutableRawPointer(dynamicUniformBuffer.contents() + uniformBufferOffset).bindMemory(to:ShaderTypes.UniformsArray.self, capacity:1)
    }
    ...
    private func updateGameState(drawable: LayerRenderer.Drawable, deviceAnchor: DeviceAnchor?) {
        ...
        func uniforms(forViewIndex viewIndex: Int) -> ShaderTypes.Uniforms {
            let view = drawable.views[viewIndex]
            let viewMatrix = (simdDeviceAnchor * view.transform).inverse
            let projection = drawable.computeProjection(viewIndex: viewIndex)

            return ShaderTypes.Uniforms(projectionMatrix: projection, modelViewMatrix: viewMatrix * modelMatrix)
        }
